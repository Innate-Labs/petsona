...
export function createHarness(emitLine: (line: string) => void) {
  ...
  const getUserPersona = () => {
    const persona = config.getPersona() as { customDescription?: unknown }
    return typeof persona.customDescription === 'string' ? persona.customDescription : ''
  }
  ...
  const heartbeat = new Heartbeat({
    ...
    decide: (input) => decideProactive(gateway, {
      ...input,
      personaCore: (() => {
        const seg = buildSegments(paths, emotion, getUserPersona())
        return [seg.personaCore, seg.userPersona].filter(Boolean).join('\n\n')
      })(),
    }),
