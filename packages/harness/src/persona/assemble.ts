...
export function buildSegments(paths: DataPaths, emotion: Emotion, userPersona = ''): PersonaSegments {
  return {
    personaCore: readIfExists(join(paths.personas, 'PERSONA.md')) || '你是桌面宠物「宠格」，说话简短温暖。',
    userPersona: wrapIfPresent('user-persona', userPersona.trim()),
...
export function assembleSystem(seg: PersonaSegments): { system: string; cacheKey: string } {
  const ordered = [seg.personaCore, normalizeUserPersona(seg.userPersona), seg.companionRules, seg.skillIndex, seg.memoryIndex, seg.emotionState, seg.timeScene]
...
function normalizeUserPersona(content: string): string {
  const trimmed = content.trim()
  if (!trimmed) return ''
  if (trimmed.startsWith('<user-persona>')) return trimmed
  return wrapIfPresent('user-persona', trimmed)
}
