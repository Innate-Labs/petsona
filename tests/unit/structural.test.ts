...
describe('UI 迁移：保留 demo_v0.1 视觉资产但不替换壳契约', () => {
  it('shell UI 引入 demo 视觉层入口与资源目录', () => {
    const uiRoot = join(ROOT, 'apps/shell/ui')
    const styles = readFileSync(join(uiRoot, 'styles.css'), 'utf8')
    const floatChat = readFileSync(join(uiRoot, 'float/FloatChat.tsx'), 'utf8')
    const petWindow = readFileSync(join(uiRoot, 'pet/PetWindow.tsx'), 'utf8')

    expect(existsSync(join(uiRoot, 'assets/demo-chat-icons/头像.png'))).toBe(true)
    expect(styles).toContain('pet-stage')
    expect(styles).toContain('chat-shell')
    expect(styles).toContain('panel-window')
